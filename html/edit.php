...
<body class="bg-primary">
    ...
    <div class="pt-5 container bg-white rounded">
        ...
        echo "<script> let Artisan = [" . json_encode($Art[0]) . "];</script>";
            ...
            <div class="card-body bg-light">
                <div class="form-row">
                    <div class="form-group col-md-3">
                        <label for="readOnlyEq">Equipment Type</label>
                        <input type="text" id="readOnlyEq" value=" <?php echo $Eq[0][0]['EquipmentType'] ?>" class="form-control" readonly>
                    </div>
                    <div class="form-group col-md-3">
                        <label for="readOnlyEd">Equipment</label>
                        <input type="text" id="readOnlyEd" value=" <?php echo $Eq[0][0]['EquipmentDescription'] ?>" class="form-control" id="Equipment" readonly>
                    </div>
                    <div class="form-group col-md-3">
                ...
                <form method="POST">
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group col-md-4">
                            ...
                            <select type="text" class="form-control" id="ComponentDiscipline" name="ComponentDiscipline" onchange="filterFailure(this)" required>
                                <?php
                                if ($Eq[0][0]['DisciplineId'] === NULL) {
                                    echo '<option value="">Select Component First</option>';
                                }
                                if ($Eq[0][0]['DisciplineId'] !== NULL) {
                                    foreach ($Des[0] as $DesRec) {
                                        if ($DesRec['DisciplineId'] === $Eq[0][0]['DisciplineId']) {
                                            echo '<option value="' . $DesmRec['DisciplineId'] . '" selected>' . $DesRec['DisciplineDescription'] . '</option>';
                                        } else {
                                            echo '<option value="' . $DesRec['DisciplineId'] . '">' . $DesRec['DisciplineDescription'] . '</option>';
                                        }
                                    }
                                }
                                ?>
                                <?php echo '<script>let DisciplineOpt="' . $Des[0][0]['DisciplineId'] . '"</script>'; ?>
                            </select>
                        </div>
                        <div class="form-group col-md-4">
                            ...
                            <select type="text" class="form-control" id="Failure" Name="Failure" required>
                                <?php
                                if ($Eq[0][0]['FailureId'] === NULL) {
                                    echo '<option value="">Select Discipline First</option>';
                                }
                                if ($Eq[0][0]['FailureId'] !== NULL) {
                                    foreach ($Fail[0] as $FailRec) {
                                        if ($FailRec['FailureId'] === $Eq[0][0]['FailureId']) {
                                            echo '<option value="' . $FailRec['FailureId'] . '" selected>' . $FailRec['FailureDescription'] . '</option>';
                                        } else {
                    ...
                    <div class="form-row">
                        <div class="form-group col-md-12">
                            ...
                            <input type="text" value="<?php echo $Eq[0][0]['DamageDetail'] ?>" class="form-control" name="DamageDetail" id="DamageDetail" placeholder="Damage details in full..">
                        </div>
                        <div class="form-group col-md-4">
                            <label for="ArtisanAssigned">Artisan assigned</label>
                            <select type="text" class="form-control" name="ArtisanAssigned" id="ArtisanAssigned">
                                <?php echo '<option value="' . $Eq[0][0]['ArtisanAssigned'] . '">' . $Eq[0][0]['ArtisanAssigned'] . '</option>' ?>;
                                <?php echo '<script>let ArtisanOpt="' . $Eq[0][0]['ArtisanAssigned'] . '"</script>'; ?>"
                            </select>
                            <script>
                                var select = document.getElementById("ArtisanAssigned");
                        ...
                        <div class="form-group col-md-8">
                            ...
                            <input type="text" value="<?php echo $Eq[0][0]['REQNumber'] ?>" class="form-control" name="REQNumber" id="REQNumber" placeholder="REQ number with delivery date">
                        </div>
                    </div>
                    ...
                    <div class="form-row">
                        <div class="form-group col-md-12">
                            ...
                            <textarea class="form-control" cols="30" rows="10" id="progress" readonly>
